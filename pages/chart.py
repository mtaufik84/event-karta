...
def update_graph(uploaded_file, chart_type):
    if uploaded_file is not None:
        ...
        # Menampilkan debug untuk memastikan data yang dibaca dari CSV
        print("Data from CSV: ", df.head())  # Menampilkan 5 baris pertama dari DataFrame
        ...
        if 'Nama_Acara' in df.columns and 'Total_Donasi' in df.columns:
            ...
            print("Column 'Nama_Acara' or 'Total_Donasi' not found in the CSV file.")
            return go.Figure()  # Return empty figure if columns are missing
            
    return go.Figure()  # Return empty figure if no file is uploaded